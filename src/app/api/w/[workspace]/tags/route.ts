...
const schema = z.object({ name: z.string().min(1).max(50), color: z.string().max(20).optional() });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ workspace: string }> }) {
  try {
    ...
    const tag = await prisma.tag.create({
      data: { workspaceId: workspace.id, name: body.data.name, color: body.data.color ?? "blue" },
    });
